...
func payloadFromWorld(w *sim.World) Payload {
	...
	for _, c := range w.Crafts {
		...
		// The per-craft projection lives in craft_wire.go so ONE craft can be
		// persisted on its own (ADR 0040's durable dock payloads).
		p.Crafts = append(p.Crafts, CraftToWire(c))
	}
...
func worldFromPayload(p Payload, systems []bodies.System) (*sim.World, error) {
	...
	for _, wc := range wireCrafts {
		// Per-craft rehydration lives in craft_wire.go (ADR 0040) — the same
		// entrypoint a durable dock payload restores through.
		c, err := CraftFromWire(wc, systems)
		if err != nil {
			return nil, err
		}
