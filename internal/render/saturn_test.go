...
func TestSaturnPixelColorEquatorialZone(t *testing.T) {
	...
	got := SaturnPixelColor(0, 0, 32, 0, 0, 0, 1)
...
func TestSaturnPixelColorPolarHexagon(t *testing.T) {
	...
	got := SaturnPixelColor(0, dy, r, 0, 0, 0, 1)
...
func TestSaturnPixelColorMultiBand(t *testing.T) {
	...
	for dy := -r; dy <= r; dy++ {
		for dx := -r; dx <= r; dx++ {
			...
			seen[string(SaturnPixelColor(dx, dy, r, 0, 0, 0, 1))] = true
		}
	}
...
func TestSaturnPixelColorDeterministic(t *testing.T) {
	...
	for dy := -r; dy <= r; dy += 4 {
		for dx := -r; dx <= r; dx += 4 {
			a := SaturnPixelColor(dx, dy, r, 0, 17.5, 0, 1)
			b := SaturnPixelColor(dx, dy, r, 0, 17.5, 0, 1)
			if a != b {
