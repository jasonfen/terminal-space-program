...
func TestSaturnPixelColorMultiBand(t *testing.T) {
	...
	for dy := -r; dy <= r; dy++ {
		for dx := -r; dx <= r; dx++ {
			...
			seen[string(SaturnPixelColor(dx, dy, r, 0))] = true
		}
	}
...
func TestSaturnPixelColorDeterministic(t *testing.T) {
	...
	for dy := -r; dy <= r; dy += 4 {
		for dx := -r; dx <= r; dx += 4 {
			a := SaturnPixelColor(dx, dy, r, 17.5)
			b := SaturnPixelColor(dx, dy, r, 17.5)
			if a != b {
