...
func TestUranusPixelColorBands(t *testing.T) {
	// Equator → base color.
	got := UranusPixelColor(0, 0, 32, 0, 0, 0, 1)
	...
	got = UranusPixelColor(0, dy, r, 0, 0, 0, 1)
...
func TestNeptunePixelColorBandsAndSpot(t *testing.T) {
	// Equator → base.
	got := NeptunePixelColor(0, 0, 32, 0, 0, 0, 1)
	...
	for dy := -r; dy <= r; dy++ {
		for dx := -r; dx <= r; dx++ {
			...
			seen[string(NeptunePixelColor(dx, dy, r, 0, 0, 0, 1))] = true
		}
	}
...
func TestIceGiantsDeterministic(t *testing.T) {
	...
	for dy := -r; dy <= r; dy += 4 {
		for dx := -r; dx <= r; dx += 4 {
			au, bu := UranusPixelColor(dx, dy, r, 0, 7.5, 0, 1), UranusPixelColor(dx, dy, r, 0, 7.5, 0, 1)
			if au != bu {
				t.Fatalf("Uranus non-deterministic at (%d,%d)", dx, dy)
			}
			an, bn := NeptunePixelColor(dx, dy, r, 0, 7.5, 0, 1), NeptunePixelColor(dx, dy, r, 0, 7.5, 0, 1)
			if an != bn {
