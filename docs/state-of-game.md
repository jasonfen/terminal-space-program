...
### Inclined-transfer wait-vs-Δv median
<!-- llm-parse: id=inclined-transfer-wait-median status=backlog target=v0.13+ -->
🧊 **backlog · think-later (reopened 2026-06-03)**. The intra-primary `[H]` split (`splitNodePhasing`, `internal/sim/maneuver.go`) pins departure to the next craft∩Moon line-of-nodes crossing, so an **inclined** parking orbit waits up to half a Moon period for the cheapest plane change. Symptom: flying the Apollo stack off the **KSC pad** (~28.6° orbit) plants the first Hohmann **~12 days** out, vs ~4.3 h for the equatorial default craft the fresh-start tuning (`internal/sim/lunar_start.go`) is bound to. Jason declined this fork 2026-06-01, reopened it 2026-06-03 after hitting the Apollo path, and chose the **planner wait-vs-Δv tradeoff** lever (over retuning the start clock — defeated by launch-time RAAN — or widening the default lead).

**Direction:** a tunable knob (max acceptable wait, or max extra Δv) that lets the split depart **sooner** by paying a **moderate** plane change instead of the node-aligned ideal. **Trap to avoid:** "just prefer the combined fused-Lambert when split waits too long" does **not** work — combined is ~13.8 km/s for inclined LEO→Luna (Lambert near-180° singularity + plane change at LEO speed), far over the ~6 km/s budget. A real median needs a **new partial-tilt departure geometry** (move apoapsis toward where the Moon will be sooner, ~300–600 m/s extra), not a reuse of combined. Grill the geometry before slicing; surface the tradeoff in the HUD. Adjacent to the still-open lunar-capture inclination trim (GH #68).
...
| 13 | [HUD overlay refactor](#hud-overlay-refactor-chips--settings-screen) | L | 🧊 sliced → v0.13 (ADR 0010) | Slim always-on HUD column + compact canvas chips + Settings screen (persisted) + F2 declutter. Kills the orbit-HUD overflow/scroll. Grilled 2026-06-03; **headline of [`docs/v0.13-plan.md`](v0.13-plan.md)** (4-slice chain). |
