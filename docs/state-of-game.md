...
<!-- llm-parse: id=plane-shift-hohmann status=designed target=v0.12 weight=XL adr=0005 slice=v0.12-slice-5 -->
📐 **designed · target v0.12 (Slice 5) · XL**. Grilled 2026-05-28 (`/grill-with-docs`); design frozen in [`docs/v0.12-plan.md`](v0.12-plan.md) §Slice 5 + [`docs/adr/0005`](adr/0005-combined-plane-shift-hohmann-via-lambert.md). Resolution: a **fused single-rev Lambert** replaces the analytic intra-primary `[H]` (eccentric departure falls out for free); a new **`BurnVector`** fixed-inertial burn mode carries the 3D departure Δv; the planner computes **both** a combined transfer and a split (plane-change at transfer apoapsis) and auto-picks the cheaper; the predicted-trajectory renderer densifies around the now-known encounter. Scope held to intra-primary only (heliocentric prograde-fudge retained). **Historical spec below.** Lambert solver constrained on post-capture inclination so departure geometry lands prograde at the destination instead of the current "match ecliptic, hope arrival inclination is OK" pattern. The v0.8-plan.md retrospective explicitly flags this as **substantial** — the binding technical work is the constrained Lambert variant (root-find on inclination as well as time-of-flight), not the UI plumbing. Pairs naturally with the [capture-direction toggle](#capture-direction-toggle) since both touch arrival-side geometry.
