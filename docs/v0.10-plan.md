...
<!-- llm-parse: cycle=v0.10 status=closed anchor=manual-flight-fidelity -->
...
| v0.10.1 | **shipped** | `v0.10.1` | Shipped 2026-05-19 (PR #56 → `main` `0a278cf`, tag `v0.10.1`). Staging chain (L) — named stage catalog + multi-tier loadouts (Apollo-Stack w/ payload separation) + spawn-form stack configurator. No sim change, no save migration. See slice below. |
| v0.10.2 | **shipped** | `v0.10.2` | Shipped 2026-05-21 (PR #57 → `main` `7be5805`, tag `v0.10.2`). Rendezvous tooling (M) — actionability layer on v0.9.3: achievable-vs-current CA, Δv-to-improve, `K` plants recommended nudge. See slice below. |
| v0.10.3 | **shipped (tag local-only)** | `v0.10.3` | Tagged 2026-05-22 at `e45c1ca` — predictor adaptive sampling (M) + transfer-leg sub-step cap. Tag is local-only and intentionally unpushed: v0.10.3's commits landed on `main` bundled into PR #58 (v0.10.4), so no separate GoReleaser run; v0.10.4+ binaries already contain the work. Tag exists for sequence-completeness. See slice below. |
| v0.10.4 | **shipped** | `v0.10.4` | Shipped 2026-05-22 (PR #58 → `main` `7deb2f9`, tag `v0.10.4`). Inclination-burn true plane change (M) — `BurnPlaneChange` mode + plane-match fold-in (signed Δi, rotates plane *coincident* with target's, not just inclination magnitude). Playtest-triggered. See slice below. |
| v0.10.5 | **shipped** | `v0.10.5` | Shipped 2026-05-23 (PR #59 → `main` `5bc697b`, tag `v0.10.5`). Multi-rev porkchop UI + Lambert short/long branch picker (S) — `o`/`n`/`r`/`b` sub-menu in porkchop screen; TOF axis auto-scales by `nRev`. See slice below. |
| v0.10.6 | **shipped** | `v0.10.6` | Shipped 2026-05-23 (PR #60 → `main`, tag `v0.10.6`). Perspective tilt for orbit view (M, render+sim) — `ViewTilted` + far-side dashing + tilted-world-basis fallback + ColorBodyOrbit + shift+↑/↓ θ controls. Grilled spec in this doc captures deltas vs original design pass. See slice below. |
| v0.10.7 | **shipped** | `v0.10.7` | Shipped 2026-05-23 (PR #61 → `main` `119e55a`, tag `v0.10.7`). Launch-anchor for `ViewTilted` (S, sim+render) — yaw-lock to local-vertical while apoapsis ≤ 200 km, guarded behind `ViewMode == ViewTilted`. `sim.LaunchAnchorPhi` + `sim.LaunchMissionFloorM` hoist + render-side override in `viewBasis` + `/anchor` HUD suffix; four tests green. **Closes v0.10.** See slice below. |
